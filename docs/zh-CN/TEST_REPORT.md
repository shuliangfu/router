...
### 1. 客户端路由浏览器测试 (client-browser.test.ts) - 27 项
...
- ✅ 动态路由、查询参数、组件加载器
- ✅ 启动链接拦截（同源、外链、target=_blank、修饰键等）
- ✅ destroy

### 2. 客户端路由单元测试 (client.test.ts) - 69 项
...
@dreamer/router 共 130 项测试全部通过，覆盖率为 100%。支持 Preact 与 React 渲染引擎，涵盖服务端路由扫描/匹配与客户端路由导航/守卫/链接拦截。

---

**报告生成时间**：2026-02-03 **测试环境**：Deno + Bun + Puppeteer
